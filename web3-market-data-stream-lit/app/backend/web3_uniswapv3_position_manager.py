...
# from contract_abis import uniswapv3_nft_abi
erc721_abi = [
    {
        "constant": True,
...
# Fetch all tokenIds owned by the Uniswap V3 Manager itself
...
print(f"Token IDs held by {owner_address}: {token_ids}")

# Loop through token IDs and fetch position data
positions = {}
...
for token_id in token_ids:
    try:
        ...
        contract = web3.eth.contract(address=UNISWAP_V3_MANAGER_ADDRESS, abi=uniswapv3_nft_abi)

        # Example: Fetch position details for token ID 123456
        ...
        positions[token_id] = {
            ...
            "feeGrowthInside0LastX128": str(position[8]),
            "feeGrowthInside1LastX128": str(position[9]),
            "tokensOwed0": str(position[10]),
            "tokensOwed1": str(position[11]),
        }

        # Print unpacked position data
        print(f"Token0: {position[2]}")
        print(f"Token1: {position[3]}")
        print(f"Fee Tier: {position[4]}")
        print(f"Tick Range: {position[5]} -> {position[6]}")
        print(f"Liquidity: {position[7]}")
        print(f"Unclaimed Fees: Token0 {position[10]}, Token1 {position[11]}")
    except Exception as e:
        print(f"Error fetching position for token ID {token_id}: {e}")
